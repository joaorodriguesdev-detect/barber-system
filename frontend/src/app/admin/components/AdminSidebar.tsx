          ...
          <div className="px-6 py-6 flex items-center justify-between">
            <div>
              <h1 className="text-white text-xl font-extrabold tracking-wide flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-violet-500 animate-pulse shadow-[0_0_10px_rgba(139,92,246,0.6)]"></div>
               ion System
              </h1>
            ...
            {(Object.keys(TAB_LABEL)).map((tab) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm rounded-xl transition-all duration-200 text-left
                  ${activeTab === tab ? 'bg-violet-500/10 text-violet-400 font-medium' : 'text-zinc-400 hover:text-zinc-100 hover:bg-white/[0.03]'}`}
              >
