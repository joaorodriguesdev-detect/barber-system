  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-[#121214] border border-white/[0.05] rounded-3xl p-6 md:p-8 mb-6">
          ...
          <form onSubmit={handlePostSubmit} className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                }} className="block w-full text-sm text-zinc-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-violet-500/10 file:text-violet-400 hover:file:bg-violet-500/20 cursor-pointer" />
              ...
              <div className="flex flex-col justify-between">
                <div>
                  ...
                  <textarea 
                    ...
                    className="bg-[#050505] border border-white/10 rounded-xl p-4 text-sm text-white w-full focus:border-violet-500 outline-none transition resize-none" 
                  />
                </div>
        ...
        {/* LISTAGEM DOS POSTS */}
        {loadingFeed ? (
          <div className="flex justify-center py-10"><div className="w-8 h-8 border-2 border-violet-500 border-t-transparent rounded-full animate-spin"></div></div>
        ) : feedPosts.length === 0 ? (
            ...
            {feedPosts.map((post: any) => (
              <div key={post.id} className="group relative rounded-2xl overflow-hidden border border-white/5 bg-[#0A0A0A] aspect-square">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition duration-300 flex flex-col justify-end p-4">
                  <p className="text-xs text-white line-clamp-2 mb-2">{post.caption}</p>
                  <button onClick={() => handleDeletePost(post.id)} disabled={deletingPostId === post.id}
                    className="bg-rose-500 hover:bg-rose-600 text-white text-[10px] font-bold py-2 rounded-lg transition disabled:opacity-50 w-full flex items-center justify-center gap-1">
                    <Trash2 size={12} />
                    {deletingPostId === post.id ? '...' : 'Excluir Foto'}
                  </button>
                </div>
              </div>
            ))}
