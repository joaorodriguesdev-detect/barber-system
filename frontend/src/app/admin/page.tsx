...
export default function AdminDashboard() {
  ...
  // ====== RENDER ======
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 flex font-sans selection:bg-violet-500/30">
      {/* SIDEBAR */}
      <AdminSidebar
        activeTab={activeTab}
                    ...
                    <div key={app.id} className="bg-[#0A0A0A] border border-white/5 p-5 rounded-2xl">
                      ...
                      <div className="flex gap-3">
                        <button onClick={() => handleChangeStatus(app.id, 'completed')} disabled={processingId === app.id}
                          className="flex-1 bg-violet-600 hover:bg-violet-500 text-white font-bold py-3 rounded-xl transition disabled:opacity-50 text-sm">
                ...
                pendingReviews.map((review) => (
                  <div key={review.id} className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-5">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-full bg-violet-500/10 flex items-center justify-center text-violet-400 font-bold text-sm">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => handleApproveReview(review.id)} disabled={processingReviewId === review.id}
                        className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-xl transition disabled:opacity-50 text-xs">
